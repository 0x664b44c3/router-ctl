...
VikinxProtocolDriver::VikinxProtocolDriver(QString busId, QObject * parent) :
    IOStreamBusDriverBase(busId, parent)
{
    auto t = new QTimer(this);
    t->setInterval(1000);
    t->start();
    connect(t, &QTimer::timeout, this, &VikinxProtocolDriver::onTimer);
    mDriverName = DRIVER_NAME;
}

bool VikinxProtocolDriver::rescanBus()
{
...
QString VikinxProtocolDriver::driverName() const
{
    return QStringLiteral("Evertz/Quartz");
}

QString VikinxProtocolDriver::driverInfo() const
{
...
void VikinxProtocolDriver::queryRouter(int addr, int level)
{
    Q_UNUSED(addr);
...
void VikinxProtocolDriver::queryAlarms(int addr, int level)
{
    Q_UNUSED(addr)
    Q_UNUSED(level)
}
...
void VikinxProtocolDriver::setXPoint(int addr, int level, int dest, int source)
{
    Q_UNUSED(addr);
    level = std::clamp(level, 0, MAX_LEVEL);
...
void VikinxProtocolDriver::checkQueue()
{
    if (!mChannel)
        return;
    if (!mHasResponse)
...
void VikinxProtocolDriver::processRx(const QByteArray & d)
{
    mBuffer.append(d);
    int offset = 0;
    mBuffer.replace('\n','\r');
...
void VikinxProtocolDriver::parseLine(const QByteArray &l)
{
    mHasResponse = true;
...
void VikinxProtocolDriver::onTimer()
{
...
void VikinxProtocolDriver::onChannelConnected()
{
    mCommandQueue.clear();
    mCommandQueue << ".#01";
    rescanBus();
...
void VikinxProtocolDriver::onChannelDisconnected()
{
